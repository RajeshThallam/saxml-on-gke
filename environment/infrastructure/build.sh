...
pushd ${REPO_ROOT_DIR}/environment/infrastructure
source vars.env
...
gcloud builds submit \
  ...
  --substitutions _TF_STATE_BUCKET=$TF_STATE_BUCKET,_TF_STATE_PREFIX=$TF_STATE_PREFIX,_REGION=$REGION,_ZONE=$ZONE,_MODEL_REPOSITORY_BUCKET_NAME=$MODEL_REPOSITORY_BUCKET_NAME,_SAXML_ADMIN_BUCKET_NAME=$SAXML_ADMIN_BUCKET_NAME,_NETWORK_NAME=$NETWORK_NAME,_SUBNET_NAME=$SUBNET_NAME,_CLUSTER_NAME=$CLUSTER_NAME,_NAMESPACE=$NAMESPACE,_TPU_TYPE=$TPU_TYPE,_NUM_TPU_POOLS=$NUM_TPU_POOLS,_ARTIFACT_REGISTRY_NAME=$ARTIFACT_REGISTRY_NAME \
  --timeout "2h" \
  --machine-type=e2-highcpu-32 \
  --quiet \
  --async \
  ${REPO_ROOT_DIR}/environment && \
